...
class _ViewMarineMoneyReceiptScreenState extends ConsumerState<ViewMarineMoneyReceiptScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<MarineReceiptState>(marineReceiptViewModelProvider, (previous, next) {
    ...
    return Scaffold(
          ...
          Expanded(
            child: state.isLoading
                ...
                : state.receipts.isEmpty
                    ? _buildEmptyState()
                    : RefreshIndicator(
                        onRefresh: () => ref.read(marineReceiptViewModelProvider.notifier).fetchReceipts(),
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            if (searchController.text.isNotEmpty) {
                              ...
                              final mrId = receipt.id.toString();
                              if (!policyholder.contains(search) && !mrId.contains(search)) {
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => Navigator.pushNamed(context, AppRouter.createMarineMoneyReceipt),
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              child: Column(
                ...
                Text(
                  receipt.marinebill?.marineDetails?.policyholder ?? 'N/A',
                  style: GoogleFonts.poppins(
                    fontSize: 17,
                    fontWeight: FontWeight.bold,
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        receipt.marinebill?.marineDetails?.bank?.name ?? 'N/A',
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        ...
                        Text(
                          'TK ${NumberFormat('#,##,###').format(receipt.marinebill?.grossPremium ?? 0)}',
                          style: GoogleFonts.poppins(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                    ...
                    Row(
                      children: [
                        ...
                        _buildActionBtn(Icons.edit_rounded, Colors.orange, () => Navigator.pushNamed(context, AppRouter.createMarineMoneyReceipt, arguments: receipt)),
                        const SizedBox(width: 12),
  ...
  void _showReceiptDetails(MarineMoneyReceipt receipt) {
            ...
            children: [
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    Text(
                      'Marine Receipt Details',
                      style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.bold),
                    ),
                    const SizedBox(height: 24),
                    _buildDetailRow('Client Name', receipt.marinebill?.marineDetails?.policyholder),
                    _buildDetailRow('Bank Name', receipt.marinebill?.marineDetails?.bank?.name),
  ...
  void _confirmDelete(int id) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Delete this marine receipt?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(context);
              final success = await ref.read(marineReceiptViewModelProvider.notifier).deleteReceipt(id);
              if (success && mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
